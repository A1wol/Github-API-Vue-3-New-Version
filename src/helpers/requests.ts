...
export async function getCurrentRepositoryDetailsResponse(repositoryID: string) {
    return await Axios.get<TRepository>(`https://api.github.com/repositories/${repositoryID}`)
}

export async function getGithubRepositoriesResponse(searchParams: string) {
    return await Axios.get<TRepository>(`https://api.github.com/search/repositories?${searchParams}`)
}